...
pub(super) struct StackdriverLabelConfig {
    /// The value of this field is used to find the related labels from jsonPayload
    /// and extract the value of it to set the LogEntry Labels
    #[serde(skip, default = "default_labels_key")]
...
fn labels_examples() -> HashMap<String, String> {
    ...
    example.insert(
        "user_label_2".to_string(),
        "{{ template_value_2 }}".to_string(),
    );
    example
}
